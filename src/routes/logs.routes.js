...
<head>
  ...
  <style nonce="${nonce}">
    ...
    *{box-sizing:border-box}body{font:14px system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;margin:0;min-height:100vh;color:var(--ink);background:#e8ebf5}
    ...
    .layout{display:block}.tableWrap{overflow:auto}.toolbar{display:flex;justify-content:space-between;align-items:center;padding:12px 14px;border-bottom:1px solid var(--line);gap:10px}.toolbar .left,.toolbar .right{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
    table{width:100%;border-collapse:collapse;min-width:960px}th,td{border-bottom:1px solid #e7e8f2;padding:12px 10px;text-align:left;vertical-align:middle}th{background:#f6f7fc;color:#42436a;font-size:12px;text-transform:uppercase;letter-spacing:.03em}tbody tr:not(.selected){cursor:pointer}tbody tr:hover{background:#f5f6ff}tr.selected{background:#eef1ff}
    ...
    .side{position:fixed;z-index:30;top:0;right:0;width:min(520px,100vw);height:100dvh;overflow:auto;padding:22px;background:#fff;border-radius:0;box-shadow:-18px 0 55px rgba(20,25,55,.2);transform:translateX(105%);transition:transform .22s ease}.side.open{transform:translateX(0)}.side h2{font-size:20px;margin:0}.side-head{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:14px}.detail-close{padding:7px 10px;background:#eef0fa;color:#30336e;border:1px solid #cfd2ec}.detail-backdrop{position:fixed;inset:0;z-index:29;background:rgba(4,12,28,.45);opacity:0;visibility:hidden;transition:opacity .2s}.detail-backdrop.open{opacity:1;visibility:visible}.empty{color:var(--muted);padding:18px;text-align:center}
    ...
    @media(max-width:1050px){.filters{grid-template-columns:1fr 1fr}.actions{grid-column:1/-1}.stats{grid-template-columns:repeat(3,1fr)}.top{display:block}.live{width:max-content;margin-top:12px}}
    @media(max-width:620px){main{padding:14px;overflow-x:hidden}.filters,.stats,.lists{grid-template-columns:1fr}.toolbar{align-items:flex-start;flex-direction:column}.brand h1{font-size:23px}.brand img{width:40px;height:40px}.tableWrap{max-width:100vw}table{min-width:0}th,td{padding:10px 7px;font-size:12px}th:nth-child(5),td:nth-child(5),th:nth-child(8),td:nth-child(8),th:nth-child(9),td:nth-child(9),th:nth-child(10),td:nth-child(10),th:nth-child(11),td:nth-child(11){display:none}th:nth-child(4),td:nth-child(4){max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.method,.risk,.status{min-width:auto;padding:3px 6px;font-size:10px}.side{padding:18px}.lists{grid-template-columns:1fr 1fr}.pager{justify-content:space-between}.toolbar .right{width:100%}.toolbar .right button{flex:1}}
  </style>
</head>
<body>
  <main>
    ...
    <section class="layout">
      ...
      <aside id="detailPanel" class="panel side" aria-hidden="true"><div class="side-head"><h2>Détails de la requête</h2><button id="closeDetail" class="detail-close" type="button" aria-label="Fermer les détails">Fermer</button></div><div id="detail" class="empty">Cliquez sur une requête pour consulter ses détails.</div><div class="lists"><div class="mini panel"><h3>Top ressources</h3><ol id="resources"></ol></div><div class="mini panel"><h3>Methodes</h3><ol id="methods"></ol></div></div></aside>
    </section>
    ...
    <script nonce="${nonce}">
      ...
      const rows=el('rows');
      function openDetail(){el('detailPanel').classList.add('open');el('detailPanel').setAttribute('aria-hidden','false');el('detailBackdrop').classList.add('open');document.body.style.overflow='hidden'}
      function closeDetail(){el('detailPanel').classList.remove('open');el('detailPanel').setAttribute('aria-hidden','true');el('detailBackdrop').classList.remove('open');document.body.style.overflow=''}
      ...
      function showDetail(x){const detail=el('detail');detail.className='detail';detail.replaceChildren();const meta=x.metadata||{};const items=[['ID',x.id],['Date',new Date(x.created_at).toLocaleString()],['Utilisateur',textUser(x)],['Email',x.User?.email||''],['Methode',x.method],['Chemin',x.path],['Ressource',x.resource],['Statut',x.status_code],['Risque',x.risk],['IP publique',x.ip_address||''],['User-agent',x.user_agent||'']];if(meta.inputs)items.push(['Inputs',JSON.stringify(meta.inputs,null,2)]);if(meta.output)items.push(['Output',JSON.stringify(meta.output,null,2)]);if(meta.headers)items.push(['Headers',JSON.stringify(meta.headers,null,2)]);if(!meta.inputs&&!meta.output)items.push(['Metadata',JSON.stringify(meta,null,2)]);items.forEach(([label,value])=>{const box=document.createElement('div');const small=document.createElement('span');small.textContent=label;const strong=document.createElement('strong');if(['Inputs','Output','Headers','Metadata'].includes(label)){const pre=document.createElement('pre');pre.textContent=value;pre.style.margin='0';pre.style.whiteSpace='pre-wrap';pre.style.wordBreak='break-word';pre.style.fontSize='12px';strong.append(pre)}else{strong.textContent=value}box.append(small,strong);detail.append(box)});openDetail()}
      ...
      el('apply').addEventListener('click',applyFilters);el('refresh').addEventListener('click',load);el('reset').addEventListener('click',()=>{['q','method','statusRange','from','to'].forEach((id)=>{el(id).value=''});state.page=1;state.selected.clear();load()});el('prev').addEventListener('click',()=>{if(state.page>1){state.page-=1;load()}});el('next').addEventListener('click',()=>{if(state.page<state.totalPages){state.page+=1;load()}});el('selectAll').addEventListener('click',()=>{state.logs.forEach((x)=>state.selected.set(String(x.id),x));renderRows();el('statSelected').textContent=state.selected.size});el('clearSelection').addEventListener('click',()=>{state.selected.clear();renderRows();el('statSelected').textContent=0;el('detail').className='empty';el('detail').textContent='Cliquez sur une requete pour consulter ses details.';closeDetail()});el('masterCheck').addEventListener('change',(event)=>{state.logs.forEach((x)=>{if(event.target.checked)state.selected.set(String(x.id),x);else state.selected.delete(String(x.id))});renderRows();el('statSelected').textContent=state.selected.size});el('copySelected').addEventListener('click',async()=>{const data=Array.from(state.selected.values());await navigator.clipboard.writeText(JSON.stringify(data,null,2));el('statusText').textContent=data.length+' entree(s) copiee(s)'});el('exportCsv').addEventListener('click',()=>{location.href=exportUrl('csv')});el('exportJson').addEventListener('click',()=>{location.href=exportUrl('json')});['q','method','statusRange','from','to','limit'].forEach((id)=>el(id).addEventListener('change',applyFilters));el('closeDetail').addEventListener('click',closeDetail);el('detailBackdrop').addEventListener('click',closeDetail);document.addEventListener('keydown',(event)=>{if(event.key==='Escape')closeDetail()});
      load();
